...
function renderStudyMap(map) {
  ...
  const labelRects = [];
  const overlaps = (a, b) => !(
    a.right + 4 < b.left || a.left > b.right + 4 ||
    a.bottom + 3 < b.top || a.top > b.bottom + 3
  );
  const placeLabel = (label, x, y) => {
    const labelWidth = Math.max(24, Array.from(label).reduce((width, char) =>
      width + (/^[\x00-\xff]$/.test(char) ? 7 : 12), 0));
    const candidates = [
      [10,-10,"start"],[-10,-10,"end"],[10,17,"start"],[-10,17,"end"],
      [0,-18,"middle"],[0,25,"middle"],[18,3,"start"],[-18,3,"end"],
      [18,-24,"start"],[-18,-24,"end"],[18,31,"start"],[-18,31,"end"],
      [0,-36,"middle"],[0,43,"middle"]
    ];
    let best;
    candidates.forEach(([dx, dy, anchor], order) => {
      const textX = x + dx;
      const textY = y + dy;
      const left = anchor === "start" ? textX : anchor === "end" ? textX - labelWidth : textX - labelWidth / 2;
      const rect = {left, right:left + labelWidth, top:textY - 13, bottom:textY + 3};
      const outside = Math.max(0, padding - rect.left) + Math.max(0, rect.right - (width-padding)) +
        Math.max(0, padding - rect.top) + Math.max(0, rect.bottom - (height-padding));
      const collisions = labelRects.reduce((count, placed) => count + (overlaps(rect, placed) ? 1 : 0), 0);
      const score = collisions * 1000 + outside * 100 + order;
      if (!best || score < best.score) best = {textX, textY, anchor, rect, score};
    });
  ...
  const pointMarkup = map.points.map((point, index) => {
    ...
    const {textX, textY, anchor} = placeLabel(point[0], x, y);
    return `
      <g class="map-point">
        <circle cx="${x}" cy="${y}" r="5"></circle>
        <circle cx="${x}" cy="${y}" r="10" class="map-pulse"></circle>
        <text x="${textX}" y="${textY}" text-anchor="${anchor}">${escapeMarkup(point[0])}</text>
      </g>`;
  }).join("");
  return `
